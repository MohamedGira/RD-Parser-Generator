...
def matchfunc(onething,rightSideStr):
    if(onething[0]=='<'):
        ...
        while(res[0][0]=='<'):
            ...
            print(res[0])
        return matchfunc2(rightSideStr,many)
    elif(onething[0]=='['):
...
def generate_parse_functions(rules):
    ...
    for rule in grammer:
        ...
        if(len(Ors)==1):
             ...
             s=add_line(s,f' return applyfills(matches,Children,ind,"{function_name.lower()}")')
        elif len(Ors)>1:
...
s=add_line(s,'def applyfills(matches,Children,ind,func_name):')
s=add_line(s,'    arr=[]')
s=add_line(s,'    out={}')
s=add_line(s,'    i=0')
...
s=add_line(s,'                i=matches.index(Token_type.newLine)')
