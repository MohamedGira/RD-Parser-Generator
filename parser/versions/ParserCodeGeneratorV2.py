...
def matchfunc(rightSideStr,many=False):
    ...
    if(firstelement=='<'):
        ...
        first_token=rightSideStr.split(' ')[0]
        token_name=re.sub(r'\[|]',r'',first_token).lower()
    ...
    if(firstelement=='<'):
        ...
        if(many):
            ...
            if(len(lowest_level_array)>1):
                for i in lowest_level_array:
                    i=i.split(' ')[0]
                s=add_line('',f'MatchArr({sa},ind,Tokens,False)',end='')
...
""" for i in secs:
    functions=generate_parse_functions(i.grammer)
    fname='separate\\'+i.name[1:-1]
    write_to(os.path.join(os.path.dirname(__file__),f'generatedparstcodeDev.py'),f'\n{i.name}',False)
    for i in functions:
        write_to(os.path.join(os.path.dirname(__file__),f'generatedparstcodeDev.py'),i,False)
        write_to(os.path.join(os.path.dirname(__file__),f'generatedparstcodeDev.py'),'\n',False)
...
write_to(os.path.join(os.path.dirname(__file__),f'generatedparstcodeDev.py'),a,False) """

print(generate_parse_functions('<variable_declarations> -> <variable_declaration>  <variable_declarations_dash> | ε')[0])