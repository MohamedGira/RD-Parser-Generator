...
if __name__ == "__main__":
    ...
    def ScanTest(text):
        globals.errors=[]
        globals.Tokens=[]
        globals.Tokens=find_token(text)
         #print(globals.Tokens)
        df=pandas.DataFrame.from_records([t.to_dict() for t in globals.Tokens])
        #print(df)
        try:
...
"""     arr=[]
    import os
    with open(os.path.join(os.path.dirname(__file__),'FORTRAN testcases.txt'), "r",encoding = "utf-8") as file:
        test=''
        for line in file:
            if(line[0]=='_'):
                arr.append(test)
